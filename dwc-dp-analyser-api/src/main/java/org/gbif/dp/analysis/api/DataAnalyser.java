 ...
 *
 * <p>Infrastructure concerns (JDBC URLs, memory limits, thread counts, and how
 * {@code descriptorLocation} is actually resolved to bytes — local filesystem, HDFS, a staged
 * temp directory, etc.) belong to the concrete implementation and are supplied via its
 * constructor, not this interface. This interface deliberately knows nothing about storage
 * backends: {@code descriptorLocation} is an opaque identifier, not a filesystem path.
 *
